...
const navigation = [
  ...
  { label: "پروفایل من", description: "Profile", href: "/profile", icon: UserRound },
] as const;
            ...
            return (
              <Link key={item.label} href={item.href} onClick={onNavigate} aria-current={active ? "page" : undefined}
                className={cn("group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition",
                  active ? "bg-blue-500/15 text-blue-200 ring-1 ring-blue-400/25" : "text-slate-300 hover:bg-white/6 hover:text-white")}>
                <Icon className={cn("size-4.5 shrink-0", active ? "text-blue-400" : "text-slate-500 group-hover:text-slate-300")} />
          ...
          {utilities.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={onNavigate} aria-current={active ? "page" : undefined}
                ...
                <Icon className="size-4.5" /><span>{item.label}</span>
              </Link>
            );
          })}
        </div>
      </nav>
      <div className="border-t border-white/10 p-4 text-xs text-slate-500">نسخه پایه Sprint 1A</div>
    </>
  );
}
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-100">
      ...
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/10 bg-slate-950/75 px-4 backdrop-blur-xl lg:hidden">
        ...
        <button type="button" onClick={() => setOpen(true)} aria-label="باز کردن منو" className="rounded-xl border border-white/10 p-2"><Menu className="size-5" /></button>
      </header>
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button type="button" aria-label="بستن منو" className="absolute inset-0 bg-black/65 backdrop-blur-sm" onClick={() => setOpen(false)} />
          <aside className="absolute inset-y-0 right-0 flex w-[min(88vw,20rem)] flex-col border-l border-white/10 bg-slate-950 shadow-2xl">
            <button type="button" onClick={() => setOpen(false)} aria-label="بستن منو" className="absolute left-4 top-4 z-10 rounded-lg p-2 text-slate-400 hover:bg-white/10"><X className="size-5" /></button>
